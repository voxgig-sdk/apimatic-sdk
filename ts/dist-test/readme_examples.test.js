...
const TEST_SEED = { "entity": { "transform": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ApimaticSDK\b/.test(code) ||
        /\bApimaticSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ApimaticSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ApimaticSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
