...
local TransformEntity = {}
TransformEntity.__index = TransformEntity


function TransformEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TransformEntity)
...
function TransformEntity:get_name()
  return self._name
end


function TransformEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TransformEntity.new(self._client, opts)
end
...
function TransformEntity:mark_deleted()
  self._deleted = true
end


function TransformEntity:deleted()
  return true == self._deleted
end


function TransformEntity:data_set(args)
  if args ~= nil then
...
function TransformEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TransformEntity:match_set(args)
  if args ~= nil then
...
function TransformEntity:match_get()
...
function TransformEntity:stream(action, args, callopts)
...
---@param reqdata TransformCreateData
---@param ctrl? table
---@return Transform
---@return string? err
function TransformEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function TransformEntity:_run_op(ctx, post_done)
  local utility = self._utility
