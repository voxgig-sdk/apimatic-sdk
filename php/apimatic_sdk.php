...
class ApimaticSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new ApimaticUtility();
        $this->_utility = $utility;

        $config = ApimaticConfig::shared_config();
        ...
        $feature_opts = ApimaticHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ApimaticHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ApimaticFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ApimaticFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ApimaticUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ApimaticHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ApimaticHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ApimaticHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ApimaticError($op . "_allow",
                "ApimaticSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ApimaticHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ApimaticError("graphql_error",
                "ApimaticSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Transform()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->transform()
    // resolves here too.
    public function Transform($data = null)
    {
        require_once __DIR__ . '/entity/transform_entity.php';
        if ($data === null) {
            if ($this->_transform === null) {
                $this->_transform = new TransformEntity($this, null);
            }
            return $this->_transform;
        }
        return new TransformEntity($this, $data);
    }
