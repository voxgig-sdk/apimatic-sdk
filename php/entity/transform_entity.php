...
class TransformEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TransformEntity($this->_client, $opts);
    }

    /**
     * @param Transform|array $args Transform data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ApimaticHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Transform|array The current Transform data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Transform fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ApimaticHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Transform fields).
     */
     ...
     *
     * @param TransformCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed TransformCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Transform|array The created Transform as an assoc-array at the
     *   SDK boundary; throws ApimaticError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = ApimaticHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
