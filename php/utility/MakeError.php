...
class ApimaticMakeError
{
    public static function call(?ApimaticContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ApimaticContext([], null);
        }
        $op = $ctx->op ?? new ApimaticOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ApimaticResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ApimaticError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ApimaticError('', $msg, $ctx);
        ...
        if ($err instanceof ApimaticError) {
