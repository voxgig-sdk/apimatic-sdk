...
[MCP](https://modelcontextprotocol.io) server exposing the Apimatic SDK as
two agent tools — `apimatic_list` and `apimatic_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/apimatic-mcp)
make build

# 2. Provide credentials via the environment
export APIMATIC_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user apimatic \
  -- /absolute/path/to/apimatic-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./apimatic-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// apimatic_list: first page of records
{ "entity": "transform" }
{ "entity": "transform", "query": { } }

// apimatic_load: one record by id
{ "entity": "transform", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/apimatic-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user apimatic \
     -- "$PWD"/dist/*/apimatic-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `apimatic_list` and `apimatic_load` tools now appear
   in new sessions. Ask the agent to *"list transform using apimatic"*
   and it calls `apimatic_list` with `{"entity":"transform"}`.
...
```sh
export APIMATIC_APIKEY=sk_live_xxx            # API key
export APIMATIC_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `apimatic_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `apimatic_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `APIMATIC_APIKEY` | API key sent with every request. |
| `APIMATIC_BASE` | Optional override of the API base URL. |
...
```sh
./apimatic-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"apimatic_load","arguments":{"entity":"transform","query":{"id":1}}}}'
```
