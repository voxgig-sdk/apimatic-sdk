...
import (
	...
	omni "github.com/voxgig-sdk/apimatic-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ApimaticError); is {
				out := map[string]any{
					"name":    "ApimaticError",
...
func omniCtx(arg any, client *sdk.ApimaticSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ApimaticSDK); is {
			client = live
