...
class FtCtrl extends ApimaticControl
{
...
class FtClient
{
    ...
    private ApimaticUtility $utility;

    public function __construct(ApimaticUtility $utility, array $options)
    {
    ...
    public function get_utility(): ApimaticUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ApimaticContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ApimaticContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ApimaticContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ApimaticConfig::make_feature($name);
    ...
    private static function buildUrl(ApimaticSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ApimaticError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ApimaticResponse($fetched);
        }
    ...
    private function fail(ApimaticContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ApimaticContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
