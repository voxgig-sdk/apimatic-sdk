...
{
    private static function utility(): ApimaticUtility
    {
        return new ApimaticUtility();
    }
    ...
    private static function ctx(array $over = []): ApimaticContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ApimaticContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ApimaticOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ApimaticError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ApimaticOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ApimaticMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ApimaticOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ApimaticMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ApimaticOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ApimaticMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ApimaticSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ApimaticMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ApimaticResponse([]),
            'result' => new ApimaticResult([]),
        ]);
        [, $err] = ApimaticMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ApimaticSpec([]),
            'response' => null,
            'result' => new ApimaticResult([]),
        ]);
        [, $err] = ApimaticMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ApimaticSpec(['step' => 's']),
            'response' => new ApimaticResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ApimaticResult([]),
        ]);
        [, $err] = ApimaticMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ApimaticSpec(['step' => 's']),
            'response' => new ApimaticResponse(self::resp(200, ['v' => 1])),
            'result' => new ApimaticResult([]),
        ]);
        [, $err] = ApimaticMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ApimaticSpec(['step' => 's']),
            'response' => new ApimaticResponse(self::resp(200, ['v' => 2])),
            'result' => new ApimaticResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ApimaticMakeResponse::call($ctx);
    ...
    {
        $preset = new ApimaticResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ApimaticSpec([]),
            'response' => new ApimaticResponse([]),
            'result' => new ApimaticResult([]),
        ...
        [$got, $err] = ApimaticMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ApimaticResult([])]);
        [, $err] = ApimaticMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ApimaticSpec([]), 'result' => null]);
        [, $err] = ApimaticMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ApimaticSpec(['step' => 's']),
            'result' => new ApimaticResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ApimaticOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ApimaticMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ApimaticSpec(['step' => 's']),
            'result' => new ApimaticResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ApimaticOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ApimaticMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ApimaticResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ApimaticSpec([]),
            'result' => new ApimaticResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ApimaticMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ApimaticMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ApimaticError('boom', 'boom');
        $utility->fetcher = function (ApimaticContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ApimaticSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ApimaticMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ApimaticContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ApimaticMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ApimaticResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ApimaticContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ApimaticSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ApimaticMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ApimaticResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ApimaticSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ApimaticMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ApimaticMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ApimaticMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ApimaticResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ApimaticDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ApimaticResult(['ok' => false])]);
        $this->expectException(ApimaticError::class);
        ApimaticDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ApimaticResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ApimaticMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ApimaticMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ApimaticBaseFeature();
        $b = new ApimaticBaseFeature();
        ApimaticFeatureAdd::call($ctx, $a);
        ApimaticFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ApimaticBaseFeature
    {
        $f = new ApimaticBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ApimaticFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ApimaticContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ApimaticSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ApimaticPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ApimaticResponse(['headers' => null]),
            'result' => new ApimaticResult([]),
        ]);
        ApimaticResultHeaders::call($ctx);
