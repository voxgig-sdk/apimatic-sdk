...
boru-driven command-line client **and** interactive REPL for the Apimatic
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/apimatic-cli)
make build
...
APIMATIC_BASE=https://api.example.com ./apimatic-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/apimatic-cli
   ```
...
Run with no arguments to open a REPL (prompt `apimatic>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `transform`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/apimatic-cli`. |
...
(`./apimatic-cli <expr>`) and interactively (the REPL), and expressions compose the
