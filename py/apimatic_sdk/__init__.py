...
from apimatic_sdk.utility import register

# Load features
from apimatic_sdk.feature.base_feature import ApimaticBaseFeature
from apimatic_sdk.features import _has_feature, _make_feature
...
class ApimaticSDK:
    ...
    def __init__(self, options=None):
        ...
        utility = ApimaticUtility()
        self._utility = utility

        from apimatic_sdk.config import shared_config
    ...
    def get_utility(self):
        return ApimaticUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("ApimaticSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Transform(self, data=None) -> "TransformEntity":
        """Entity factory: client.Transform().list() / client.Transform().load({"id": ...})."""
        from apimatic_sdk.entity.transform_entity import TransformEntity
        return TransformEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "ApimaticSDK":
        if sdkopts is None:
            sdkopts = {}
